...
class UncertaintyModel(nn.Module):
    def __init__(self, config):
        ...
        # Adjust Network
        self.conv1 = self.conv_block(in_features, 256, kernel_size=3, padding=1)
        self.conv2 = self.conv_block(256, 128, kernel_size=3, padding=1)
        self.bn = nn.SyncBatchNorm(128)
        self.conv_seg = nn.Conv2d(128, 1, kernel_size=1)
        nn.init.normal_(self.conv_seg.weight.data, 0, 0.01)
        nn.init.zeros_(self.conv_seg.bias.data)

        # self.sky_conv1 = nn.Conv2d(1, 16, kernel_size=3, padding=1)
        # self.sky_conv2 = nn.Conv2d(16, 32, kernel_size=3, padding=1)
        # self.sky_conv3 = nn.Conv2d(32, 1, kernel_size=3, padding=1)
        ...
        # self.conv1_extra = nn.Conv2d(1, 64, kernel_size=3, padding=1)
        # self.conv2_extra = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        # self.conv3_extra = nn.Conv2d(128, 1, kernel_size=3, padding=1)


        # TODO: currently support seq 022 of waymo only
        img_norm_mean = torch.tensor([0.3609696,  0.38405442, 0.4348492], dtype=torch.float32)
        img_norm_std = torch.tensor([0.19669543, 0.20297967, 0.22123419], dtype=torch.float32)
        ...
        self.idx = 0
        self.uncertainty=[]
        self.save_dir = os.path.join(config.visualize_dir,config.uncertainty_stage)
        loss_mult_dir=os.path.join(self.save_dir,'loss_mult')
        uncertainty_dir=os.path.join(self.save_dir,'uncertainty')
        vis_dir=os.path.join(self.save_dir,'vis')
        os.makedirs(self.save_dir, exist_ok=True)
        os.makedirs(loss_mult_dir,exist_ok=True)
        os.makedirs(uncertainty_dir,exist_ok=True)
        os.makedirs(vis_dir, exist_ok=True)

        self._load_model()

    def forward_sky(self, x):
        x = F.relu(self.sky_conv1(x))
        x = F.relu(self.sky_conv2(x))
        x = torch.sigmoid(self.sky_conv3(x))
        return x
    ...
    def _compute_losses(self, gt, prediction, sky_mask=None, prefix='', _cache_entry=None):
        uncertainty = self(self._scale_input(gt, self.config.uncertainty_dino_max_size), _cache_entry=_cache_entry)
        # sky_mask = sky_mask.to(torch.float32)
        # sky_mask=self.forward_sky(sky_mask)
        # sky_mask = (sky_mask >= 0.5).bool()

        # if sky_mask is not None:
        #     uncertainty[sky_mask]=self.config.uncertainty_clip_min

        # extra_conv
        # x = F.relu(self.conv1_extra(uncertainty))
        # x = F.relu(self.conv2_extra(x))
        # uncertainty = self.conv3_extra(x)

        # Saving
        if self.idx%100==0:
            uncertainty_num=uncertainty.cpu().detach().numpy()
            np.save(os.path.join(self.save_dir, 'uncertainty', f'{self.idx:06d}_uncertainty.npy'), uncertainty_num)
            cv2.imwrite(os.path.join(self.save_dir, 'vis', f'{self.idx:06d}_uncertainty.png'),normalize_to_255(uncertainty_num.squeeze()))
        ...
        if self.config.uncertainty_mode == "l2reg":
            ...
            if self.idx%100==0:
                ...
                cv2.imwrite(os.path.join(self.save_dir,'vis',f"{self.idx:06d}_loss_mult0.png"),normalize_to_255(loss_mult_num.squeeze()))


            gt_down = dino_downsample(gt, max_size=952)
            prediction_down = dino_downsample(prediction, max_size=952)
            ...
            # Saving
            if self.idx%100==0:
                dino_cosine_num=dino_cosine.cpu().numpy()
                cv2.imwrite(os.path.join(self.save_dir,'vis',f"{self.idx:06d}_dino_cosine.png"),dino_cosine_num.squeeze()*255.0)
            dino_part = (1 - dino_cosine.sub(0.5).div(0.5)).clip(0.0, 1)
            ...
            if self.idx%100==0:
                dino_part_num=dino_part.cpu().numpy()
                cv2.imwrite(os.path.join(self.save_dir,'vis',f"{self.idx:06d}_dino_part.png"),dino_part_num.squeeze()*255.0)
            
            uncertainty_loss = dino_part * dino_downsample(loss_mult, max_size=952)
    ...
    def get_loss(self, gt_image, image, sky_mask=None,prefix='', _cache_entry=None):
        gt_torch = gt_image.unsqueeze(0)
        image = image.unsqueeze(0)
        sky_mask=sky_mask.unsqueeze(0)
        loss, metrics, loss_mult = self._compute_losses(gt_torch, image,sky_mask, prefix, _cache_entry=_cache_entry)
    ...
    def load(path: str, config: OmegaConf) -> "UncertaintyModel":
        ckpt = torch.load(os.path.join(path, "uncertainty_checkpoint.pth"), map_location="cpu")
