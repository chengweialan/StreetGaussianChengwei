    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, viewpoint in enumerate(tqdm(config['cameras'], bar_format="{l_bar}{bar:50}{r_bar}")):
                ...
                print(f"rendered:{image.device},not:{gt_image.device}")
                cv2.imwrite(os.path.join(outdir, f"{viewpoint.colmap_id:03d}_gt_image.png"), (gt_image[[2,1,0], :, :].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8))

                cv2.imwrite(os.path.join(outdir, f"{viewpoint.colmap_id:03d}_rendered_image.png"), (image[[2,1,0], :, :].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8))
